...
function loadFromJson() {
  ...
  if (existsSync("./_redirects.json")) {
    ...
    log.warn(`${cliNow()} <cyan>redirectsMiddleware</cyan>: No './_redirects.json' found.`);
  }
...
function addGoLinksAndRedirectLinks(redirects: Record<string, string>) {
  log.debug(`${cliNow()} addGoLinksAndRedirectLinks: Adding additional redirects...`);

  redirects["/api/"] = "/api/deno/";

  log.debug(`${cliNow()} <cyan>redirectsMiddleware</cyan>: Reading redirects from 'go.json'...`);
  for (const [name, url] of Object.entries(GO_LINKS)) {
    redirects[`/go/${name}/`] = url;
  }

  log.debug(`${cliNow()} <cyan>redirectsMiddleware</cyan>: Reading redirects from 'oldurls.json'...`);
  for (const [name, url] of Object.entries(REDIRECT_LINKS)) {
    redirects[name] = url;
  }

  log.warn(
    `${cliNow()} <cyan>addGoLinksAndRedirectLinks</cyan>: Total number of redirects loaded: <green>${Object.keys(redirects).length}</green>.`,
  );
}
...
export function toFileAndInMemory(redirects: Redirect[], site: Site): void {
  ...
  log.warn(`${cliNow()} <cyan>toFileAndInMemory</cyan> Added <green>${redirects.length}</green> redirects.`);
