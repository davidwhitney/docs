...
const SKIP_CHECK_URLS = (Deno.env.get("SKIP_CHECK_URLS") || "false").toLowerCase();
...
if (SKIP_CHECK_URLS !== "true") {
  ...
  log.warn(`${cliNow()} <cyan>checkUrls</cyan>: Skipping broken link check for local performance.`);
}

site.addEventListener("afterStartServer", () => {
  log.warn(`${cliNow()} Server available at <green>http://localhost:3000</green>`);
});
